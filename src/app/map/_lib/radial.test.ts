 ...
 * RING_START 주석) 5건 중 4건이 초록으로 돌아왔다 — 한도 50명의 점-점 간격
 * 한 건은 여전히 빨간불이다(아래 NODE_THRESHOLD 주석 참고). 284px 에서는
 * 다섯 상수 어느 조합으로도 손댈 수 없던 4.3px 부족이 0.28px 까지 좁혀졌다.
 */
 ...
 * 문턱이 둘인 이유.
 *
 * 모바일 375px 폭에 한도 50명을 넣고 19px(NO_OVERLAP_PX)를 지키는 것은 이
 * 배치로는 안 된다(아래 ⚠️ 참고). 그래서 약속을 둘로 나눈다. 흔한 지도
 * (시드 25명)는 편안한 거리(COMFORT_NODE_PX=22px)를 보장하고, 한도까지 채운
 * 지도·한쪽에 몰린 지도는 **적어도 겹치지 않는다**(NO_OVERLAP_PX=19px). 뒤쪽은
 * 약해진 약속이지만 여전히 진짜 약속이다 — 그리고 그런 지도는 카메라를
 * 당겨서 본다.
 *
 ...
 *
 * ⚠️ 한도 50명만은 여전히 빨간불이다: 375×420 에서 가장 가까운 점 쌍이
 * 18.72px 로, NO_OVERLAP_PX(19px)에 0.28px 모자란다(284px 시절의 4.3px
 * 부족과 비교하면 병목의 대부분은 사라졌다). 다섯 상수를 폭넓게(좌표 하강 +
 * 무작위 재시작, 이 값 하나만을 목적함수로 삼아 최대화해도) 다시 훑어도
 * 18.7~18.9px 부근에서 멈춘다 — 375px 폭에서 50명을 이 배치(다섯 슬롯 ×
 * 세 링, 열 간격 MIN_GAP)로 담는 것 자체의 구조적 상한으로 보인다
 * (radial.ts 의 RING_START 주석도 같은 결론). 문턱을 낮추는 대신 실패를
 * 그대로 남긴다.
 */
