...
/**
 * 나만 노드가 크다. 코어와 근접 halo 에 함께 걸린다. 확산 halo 는 모두가 같다.
 *
 * 진입 화면(375px, 1 월드 단위 ≈ 32.5px) 기준으로 4 → 5.5 로 올렸다:
 *   코어 지름   0.075×2×5.5 = 0.825 월드 = 26.8px  (예전 19.5px)
 *   근접 halo   0.28 ×2×5.5 = 3.08  월드 = 100.1px (예전 72.8px)
 *
 * 사람의 코어가 3.9~6.7px 라 배율은 그대로 "중심이 얼마나 큰가" 다. 더 올리지
 * 않는 이유는 근접 halo 반지름(50px)이 이미 이름표 자리를 먹고 있어서다 —
 * SelfCore 의 명패 오프셋이 이 값에서 따라온다.
 */
export const SELF_NODE_SCALE = 5.5;
