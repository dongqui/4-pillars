...
type Props = Pick<ReportContent, "headline" | "summary"> & {
  ...
  return (
    <section className="text-center">
      ...
      <div className="flex flex-wrap justify-center gap-2 mt-[26px]">
        {/* title 은 프롬프트에서만 "서로 겹치지 않게" 부탁할 뿐 스키마로 유일성을
            강제하지 않는다 — 인덱스를 키로 써서 중복 제목이 React key 충돌을
            일으키지 않게 한다. */}
        {keywords.map((k, i) => (
          <span key={i} className="text-[13px] font-semibold text-slate-700 bg-slate-100 px-[13px] py-1.5 rounded-full">{k}</span>
        ))}
      </div>
    </section>
  );
