 ...
 *
 * ⚠️ 이 숫자를 정할 때 보지 않은 상호작용 하나: 궁합은 생성 실패를 되돌리지 않는다
 * (consultations/service.ts 와 반대 — 권한이 남아 재시도가 공짜이므로 되돌릴 이유가
 * 없다). 그런데 PromptedMatchGenerator.generateSections 는 한 번의 생성마다 LLM 을
 * 5번(섹션 수만큼) 병렬로 부른다. 그러니 이미 권한을 가진 사용자가 같은 궁합을
 * 반복 재시도하면 — 섹션이 끝내 다 저장되지 않는 궁합이거나, Suspense 대기 중
 * 조급하게 새로고침하는 경우 — 시간당 최대 60 × 5 = 300 LLM 호출까지 갈 수 있다.
 * 옛 한도(5)에서는 같은 계산이 25였다. 이 파일의 기존 주석대로 이 경로에는 여전히
 * DB 계층 상한이 없다.
 */
