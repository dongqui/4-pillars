 ...
 *
 * ⚠️ 이 문자열은 8개 섹션 전부의 system 메시지에 그대로 실린다(prompt/index.ts).
 ...
 *
 * "배경 위치가 이 해의 성격을 결정하지 않는다" 규칙(아래)을 추가하며 전부가
 * 아니라 overview·months(그리고 당시 존재하던 08 변곡점)만 올렸다: 이 규칙이
 * 실제로 막는 것은 "이 해가 시작되는/마무리되는 시기" 라는, **한 해 전체의
 * 시간적 위치**를 규정하는 문장이다. 그 프레이밍을 실제로 쓰는 과업은
 * overview("이 해 전체를 관통하는 주제" 가 과업 자체)와 months(12개월로 한 해의
 * 호(弧)를 직접 그린다)뿐이다. 나머지 여섯(rising·straining·work·relating·
 * money·closing)은 힘·태도·행동처럼 국소적인 내용을 다루고, 어느 프롬프트도
 * "이 해 전체가 어떤 시점인가" 를 쓰라고 시키지 않는다 — 그래서 이 규칙이 그
 * 여섯의 실제 출력을 바꿀 가능성은 낮다고 보고 버전을 그대로 뒀다. 다음에 이
 * 블록을 고치는 사람은 같은 질문(8개 중 몇 개가 실제로 영향받는가)을 다시 던질 것.
 */
