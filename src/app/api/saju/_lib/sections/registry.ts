...
export const SECTIONS = {
  overview: {
    ...
    prompt: [
      ...
      "성향을 대표하는 서로 겹치지 않는 관점 4개를 traits 로 써라.",
      "",
      // 헤드라인은 대비 문형("A지만 B인 사람")으로 나오는데, 문형만 흉내내고 대비축을
      // 놓친 문장이 자주 나왔다 — "속은 여리고 단단하지만, 상황을 재빨리 읽는 사람"
      // 처럼 A 안에 반대말을 몰아넣으면 지만 앞뒤가 대비가 아니게 되어 모순으로 읽힌다.
      "headline 에 대비를 쓸 때는 대비되는 축을 하나만 잡고, 그 축의 양쪽을 앞뒤에 나눠 놓아라.",
      "- 축은 겉모습과 속마음, 평소와 결정적인 순간, 남에게 맞추는 면과 자기 기준 중 하나를 고른다.",
      '- 앞부분 안에 서로 반대되는 말을 겹쳐 넣지 마라. "속은 여리고 단단하지만" 처럼 한쪽에 양극단을 몰아넣으면 대비가 무너져 모순으로 읽힌다.',
      "- 뒷부분은 앞부분을 뒤집는 말이어야 한다. 앞과 무관한 능력·행동을 뒤에 놓지 마라.",
      '- 반대되는 두 성질을 함께 쓰고 싶으면 대비 대신 인과로 이어라 — "여린 만큼 남의 기색을 빨리 읽는" 처럼 하나가 다른 하나의 이유가 되게 쓴다.',
      '- 좋은 예: "겉으로는 무던해 보이지만, 한번 정한 기준은 좀처럼 굽히지 않는 사람" / "평소엔 한발 물러서 있다가, 결정적인 순간에 판을 정리하는 사람"',
      '- 나쁜 예: "속은 여리고 단단하지만, 상황을 재빨리 읽고 기준을 세우는 사람" (앞에 반대말이 겹쳐 있고, 뒤가 앞을 뒤집지 않는다)',
      "",
      "각 trait 은 세 부분이다.",
      "- title: 그 관점을 한눈에 보여주는 짧은 말. 그대로 키워드로 노출되므로 사주 용어를 쓰지 말고, 성격을 바로 알아볼 수 있는 말로 짧게 쓴다. 스무 자를 넘기지 마라.",
