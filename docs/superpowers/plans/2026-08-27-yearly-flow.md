...
describe("frictionOf — 시간 미상 보정", () => {
  it("같은 관계 구성이면 시지 유무와 무관하게 같은 크기가 나온다", () => {
    // 시지가 아무 관계도 맺지 않는 지지(축)일 때, 시지가 있고 없고가
    // friction 을 바꾸면 안 된다. 지금은 분모만 커져 시간 미상 쪽이 낮게 나온다.
...
> ⚠️ 위 테스트의 지지 조합은 `src/lib/saju-core/branch-relations.ts` 의 실제 표를 보고
> 확인할 것. `축` 이 `오`·`자`·`인`·`묘`·`신`·`유` 중 하나와 관계를 맺는다면 관계가
> 없는 다른 지지로 바꾼다 — 이 테스트의 요점은 "관계 없는 시지를 더해도 값이
> 안 변한다" 이다.
