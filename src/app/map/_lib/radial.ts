 ...
 * 값이다.
 *
 * ⚠️ 이 재조정으로도 NO_OVERLAP_PX·COMFORT_NODE_PX 를 전부 채우지는 못한다:
 * 375×420·한도 50명(FULL)의 가장 가까운 점 쌍이 18.72px 로, NO_OVERLAP_PX
 * (19px)에 0.28px 모자란다(radial.test.ts 의 NODE_THRESHOLD 주석 참고). 284px
 * 시절의 4.3px 부족과 비교하면 병목이 대부분 사라진 것이지만, 375px 라는 폭
 * 자체가 남은 병목이다 — 375×420 에서 모바일 높이를 더 올려도(700까지 실측)
 * 배율이 그대로다(아래 RING_GAP 주석 참고), 즉 폭이 다 썼다. 다섯 상수를
 * 넓게(좌표 하강 + 수백 회 무작위 재시작, FULL 의 375×420 점-점 간격만을
 * 목적함수로 삼아도) 흔들어도 18.7~18.9px 부근에서 멈췄다 — 이 폭에서 50명을
 * 담는 이 배치 자체의 구조적 상한으로 보고 여기서 멈춘다.
 */
