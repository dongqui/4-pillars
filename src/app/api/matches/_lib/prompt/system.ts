 ...
 * 같이 고쳐야 하는 상태가 된다.
 *
 * 아래 "연애 관계(썸·연인·배우자)가 아니면 …" 처럼 유형 이름이 등장하는 문장도 있다.
 * 이건 예외가 아니다 — 유형별로 다른 내용을 지시하는 것이 아니라, 열 유형 모두에게
 * 똑같이 적용되는 하나의 규칙("연애 감정을 임의로 전제하지 마라")의 **조건**이 유형
 * 이름으로 쓰였을 뿐이다. 유형마다 다른 지시(문구·관점)는 여전히 relation-copy.ts 로 간다.
 */
...
- 반대로 가족·업무·사업·선생과 제자 관계에도 그 관계에 존재하지 않는 상황을 만들지 마라. 그 관계에 실제로 존재하는 역할과 권한의 차이도 없는 것처럼 쓰지 마라.
