 ...
 * 어느 쪽도 "더 세다"가 되면 안 된다 — 이 균형을 잡던 node-visual.ts 의
 * 광량 불변식은 지워졌다. 지금은 PersonMarker.tsx 가 六合 halo 0.18, 沖 링
 * 0.6 을 고정 opacity 로 쓸 뿐이고, 이 균형을 재는 자동 검사는 없다 — 여기
 * l+4/s+12 값을 손대면 눈으로 다시 확인해야 한다.
 *
