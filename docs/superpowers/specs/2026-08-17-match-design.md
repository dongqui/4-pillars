...
**카운터를 어디서 깎는지가 구조보다 중요하다.** 참고할 파일은 두 개고, 역할이 다르다:

| 파일 | 무엇을 정하는가 |
|---|---|
| `src/lib/reports/rate-limit.ts` | 카운터의 **구조** (고정 윈도, `incr`+`expire NX`, fail-closed) |
| `src/app/report/_lib/gated-generator.ts` | 게이트의 **자리** — 생성기를 감싼다 |

게이트는 **생성기 자리**에 씌운다. `produceSections`/`produceMatchSections` 는 저장소에
없는 섹션이 있을 때만 생성기를 부르므로, 그 자리에 두면 실제로 비용이 드는 순간에만
카운터가 깎인다. `gated-generator.ts` 의 주석이 이 판단을 그대로 적고 있다.

궁합에서 자리를 틀리면 두 방향으로 다 샌다. 만들기(`POST /api/matches`)에서 세면
같은 쌍·같은 관계의 재요청은 `matches_unique` 로 기존 행에 수렴해 LLM 을 한 번도
부르지 않는데 한도를 1 먹고, 정작 5콜이 나가는 `/match/[id]` 렌더는 아무것도 세지
않아 저장이 끝나기 전 새로고침이 매번 풀 생성이 된다. 특히 어떤 섹션이 스키마 검증에
계속 실패하면 그 섹션은 저장되지 않으므로 **열 때마다** 영원히 다시 생성된다.

따라서 `canCreateMatch` 는 **세지 않고 읽기만 한다**(`peekMatchLimit`). 차감은
`src/app/api/matches/_lib/gated-generator.ts` 가 한다. 이용권이 붙어도 같은 모양이다:
잔액 확인은 `canCreateMatch`, 차감은 생성하는 자리.
...
| 오행 보완 | 내 용신·희신을 상대 원국이 몇 자 갖고 있나 (양방향) | `yongsin` + `elements` 조합 · 두 값 모두 `matchFacts` 가 소비한다 |
...
없는 합성값)를 가지면서 라벨의 근거만 더 흐려진다.

**조합은 두 축을 다 남긴다.** 배지가 분류를 대신 고르는 방식(배지가 있으면 배지 이름,
없으면 분류 이름)은 쓰지 않는다 — 그러면 `생아`+`육합`(나를 채워 주는 사람)과
`극아`+`육합`(나를 다잡는 사람)이 한 문구로 접혀 분류 축이 화면에서 사라진다.
배지는 분류를 **수식**한다: `synastryLabel` 이 분류 문구(5종) 앞에 배지 수식어(3종)를
붙여 20가지를 낸다.

```
배지 없음 + 생아  → "나를 채워 주는 쌍"
육합    + 생아  → "맞물리며 나를 채워 주는 쌍"
육합    + 극아  → "맞물리며 나를 다잡는 쌍"
충      + 비아  → "부딪히며 결이 닮은 쌍"
동일일주 + 비아  → "거울처럼 결이 닮은 쌍"
```

어휘는 `RELATION_LABELS`(귀인 · 단짝 · …) · `BADGE_LABELS`(찰떡 · 불꽃 · 쌍둥이)와
**일부러 다르다.** 그 이름들은 라벨 바로 아래 배지 줄에 근거로 붙으므로, 라벨이 같은
이름을 되풀이하면 결론 자리에 새로 얹히는 정보가 없어진다. 라벨은 서술형 한 문구,
배지는 짧은 이름 — 두 층이 각자 다른 일을 한다.
...
[사실 · 두 사람 사이]
  ...
  희신 보완: 내 희신 금 — 상대 원국에 1자 · 상대 희신 목 — 내 원국에 3자
...
CREATE TABLE IF NOT EXISTS match_sections (
  ...
  model          text   NOT NULL,
  created_at     timestamptz NOT NULL DEFAULT now(),
  -- ON CONFLICT DO UPDATE 가 갱신 시각을 남긴다. 이 컬럼이 없으면 putMatchSections 의
  -- `SET ... updated_at = now()` 이 파싱 단계에서 통째로 실패해 — 조건에 걸리는 행이
  -- 하나도 없어도 실패한다 — 아무것도 저장되지 않고 모든 열람이 5섹션을 다시 만든다.
