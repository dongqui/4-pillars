...
| 메서드 | 경로 | 권한 | 실패 |
|---|---|---|---|
| POST | `/api/maps/[share]/people` | 누구나 | 400 만세력 실패·형식 오류 / 404 없는 지도 / 409 50명 초과. **중복은 오류가 아니다** — §6.1 |
...
같은 지도에 이름·생년월일이 완전히 같은 사람이 이미 있으면, 그 사람을 **201 로 그대로
돌려준다.** 새로 넣은 경우와 응답이 구분되지 않는다.

처음 설계는 409 였다. 구현 리뷰가 그것이 **생일 추측의 신호**가 된다고 지적했고, 맞다:
지도는 링크만 있으면 누구나 열고 이름이 보이므로, `(아는 이름, 추측한 생일)` 을 POST 해서
409 면 적중으로 읽을 수 있다. 50명 한도가 탐색을 49회로 묶긴 한다 — 빗나갈 때마다 슬롯이
하나 줄고, 차고 나면 모든 추가가 한도 409 로 바뀌어 신호가 죽으며, 그 사이 지도에는 쓰레기가
훤히 쌓인다. 그래도 연·월을 이미 아는 상대라면 후보가 30개 안팎이라 그 예산 안에 들어간다.

응답을 같게 만들면 신호 자체가 사라진다. 돌려주는 것(이름·일주·구역)은 이미 지도에 보이는
것이라 새로 새지 않는다. 덤으로 추가가 멱등해져 더블탭과 재전송이 저절로 해결된다.

`map_people_dedupe` 유니크 인덱스는 그대로 둔다 — 이제는 거절의 근거가 아니라 경합에서
행이 둘로 갈리지 않게 하는 장치다.
