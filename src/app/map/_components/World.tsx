...
import { buildLayout, placePeople } from "../_lib/radial";
...
export function World({
  ...
  return (
    ...
    >
      ...
      <MapCamera layout={layout} />

      {/*
        radial.ts 는 배치를 xy 평면에 그리고 층 높이를 +z 로 준다. three 의
        조작계(OrbitControls)는 +y 가 위라고 보므로, 이 group 이 배치의
        (x, y, z) 를 월드의 (x, z, −y) 로 보낸다 — 원반이 눕고 층 높이가
        월드에서 진짜 "위" 가 된다. MapCamera 는 이 group 밖에 둔다 — 카메라는
        월드에 서 있지 지도에 실려 있지 않다.
      */}
