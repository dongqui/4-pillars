...
| 확산 halo | 1.6 | sprite, additive, α 0.07 | `glow` |
...
최종 whole-branch 리뷰에서 이 절의 최초 계산이 틀렸다는 게 드러났다. 처음엔 "사람 간 최소 간격 0.4354(`layout.test.ts` 가 잠근다)의 2배가 넘으므로 이웃과 겹친다"고 적었는데, 0.4354 는 21명 사이 210개 쌍 중 **가장 가까운 한 쌍**의 값일 뿐이고, `layout.test.ts` 가 실제로 잠그는 값은 `toBeGreaterThan(0.35)`다 — 0.4354 자체는 관측값이지 잠긴 값이 아니다. 판단에 필요한 건 최소값이 아니라 **최근접 거리의 분포**다.

21명의 실측 좌표로 각자의 최근접 이웃까지 거리를 구하면 중앙값은 **1.93**이다(최솟값 0.4354의 4.4배). 확산 halo 반지름 0.95(지름 1.9)로는 중앙값 사람의 겹치는 이웃이 **0명**이고, 21명 중 **12명**이 아무와도 겹치지 않는다 — "사람들이 Field 를 만든다"는 핵심 메커니즘이 애초에 작동하지 않았다.

반지름을 **1.6**(지름 3.2)으로 올리면 중앙값 사람은 이웃 2명과 겹치고, 고립되는 사람은 21명 중 2명뿐이다. 가장 붐비는 자리의 이웃은 최대 5명이다. additive 라 겹친 자리의 알파는 대략 `0.07 × 겹친 수` — 중앙값 자리에서 0.21(자기 것 포함 3겹), 가장 붐비는 자리에서도 6겹(이웃 5명 + 자기 자신) 0.42로 화면을 덮지 않는다.
...
아래 두 값은 처음에 계산으로 정했는데, 그 계산이 틀렸다. 최소 간격 0.4354(210개 쌍 중 가장 가까운 한 쌍)를 근거로 삼아 반지름 0.95 를 골랐지만, 판단 기준이어야 할 최근접 거리 **중앙값**은 1.93 으로 4.4배 컸다. 0.95 에서는 중앙값 사람의 겹치는 이웃이 0명, 21명 중 12명이 아무와도 겹치지 않아 "확산 halo 가 Field 를 만든다"는 핵심 메커니즘이 렌더 이전에 이미 성립하지 않았다. 최종 whole-branch 리뷰에서 이 오류가 잡혔고, 실측 분포에 맞춰 반지름을 1.6 으로 고쳤다. 여전히 렌더된 화면에서 최종 검증된 값은 아니다 — 한곳에 상수로 모아두고 실물을 본 뒤 조절한다.

```
DIFFUSE_HALO_RADIUS = 1.6
