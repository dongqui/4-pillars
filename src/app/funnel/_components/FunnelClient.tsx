...
/**
 * `initial` 은 라이트 퍼널에서 넘어온 값이다 (page.tsx 가 쿠키에서 읽어 넘긴다).
 * 캐릭터를 만들 때 이미 받은 생년월일을 리포트를 만들 때 다시 묻지 않기 위한 것이다.
 */
export function FunnelClient({ initial }: { initial?: Partial<FunnelData> }) {
  return (
    <Suspense fallback={null}>
      <FunnelProvider initial={initial}>
        <FunnelInner />
      </FunnelProvider>
    </Suspense>
  );
