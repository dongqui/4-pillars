...
const EYEBROW = "mb-[9px] text-[11.5px] font-bold tracking-[0.08em] text-slate-400";
const TITLE = "mb-[5px] text-base font-bold tracking-[-0.025em]";
const DESC = "mb-[18px] text-[13.5px] leading-[1.55] text-gray-500 [text-wrap:pretty]";
const CTA = "mt-auto whitespace-nowrap text-sm font-bold text-accent";
const ART = "pointer-events-none absolute bottom-[15px] right-4 flex items-center gap-[7px] opacity-55 md:bottom-[17px] md:right-5";
    ...
    <section className="mx-auto max-w-[780px] px-5 pb-9 pt-5 md:px-8 md:pb-[52px] md:pt-[26px]">
      ...
      <div className="grid grid-cols-1 items-stretch gap-2.5 md:grid-cols-2">
        <Link href={reportHref} className={CARD}>
          ...
          <span className={CTA}>{saved ? "리포트 보기 →" : "생년월일시 입력하기 →"}</span>
        </Link>
        ...
        <Link href="/map" className={CARD}>
          ...
          <span aria-hidden className={ART}>
            <Dot size={10} style="border-[1.5px] border-dashed border-slate-300" />
            <Dot size={13} style="bg-slate-900" />
            <Dot size={10} style="border-[1.5px] border-dashed border-slate-300" />
            <Dot size={8} style="border-[1.5px] border-dashed border-slate-200" />
          </span>
        </Link>
