...
- `/map` 은 **처음 만들 때 한 번만** 이 값을 읽는다 (`map/page.tsx` 가 지도가 없을 때만
  `primaryProfileId` 로 거른다). 이미 지도가 있으면 `getMapByOwner` 가 그 행을 그대로
  돌려주고 이 필터를 지나가지도 않는다 — `maps` 에는 UPDATE 경로가 없어(`lib/maps/store.ts`)
  중심은 첫 생성 시점에 굳는다. 그 뒤로 홈에서 무엇을 골라도 지도는 따라오지 않는다.
...
- **primary 를 지웠는데 다시 정하는 요청까지 실패하면**: `primary_profile_id` 는 null 로
  남고, 스스로 다시 채워지지 않는다. `setPrimaryProfileIfUnset` 은 "이 계정에 저장된
  프로필이 이것뿐" 일 때만 동작하는데, 다른 프로필이 이미 있으면 그 조건이 다시는
  참이 되지 않아 영구히 잠긴다. `0030` 은 마이그레이션 시점에 한 번 돌 뿐 다시 실행되지
  않는다. 고치는 길은 홈 셀렉터에서 다른 줄을 한 번 고르는 것뿐이다. 그때까지 상담·궁합은
  가장 오래된 저장 프로필로 물러서므로 화면이 깨지지는 않는다 — 실패가 아니라 낡은
  기본값일 뿐이다.
