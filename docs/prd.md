...
4-pillars/
├─ apps/
│  ├─ web/             # React Router, Vercel 배포 — locale은 런타임 Host로 판별 (kr.* → ko, jp.* → ja)
...
```txt
web     → Vercel (단일 프로젝트, kr/jp 서브도메인 매핑)
api     → Render
...
```txt
web
- 랜딩/SEO/UX: locale별 i18n 분기 (@4-pillars/i18n — ko/ja UI 카피)
- 결제 화면: locale 분기 라우트 (ko → 한국 결제, ja → 일본 결제)
- locale 판별: Host 헤더 (kr.* → ko, jp.* → ja), ?lang= 및 VITE_LOCALE 개발 오버라이드, 기본값 ko
...
Deploy
- Vercel: web (단일 프로젝트, kr/jp 서브도메인 매핑)
- Render: api
- Supabase: DB
```
...
```txt
kr.example.com   → web (Host로 locale 판별: ko)
jp.example.com   → web (Host로 locale 판별: ja)
api.example.com  → api
```
